...
export type VerifiedTorqueLink = {
  title: string;
  spec: string;
  href: string;
};
...
export default function TorqueLookupClient({
  verifiedTorqueLinks,
}: {
  verifiedTorqueLinks: VerifiedTorqueLink[];
}) {
    ...
    <main className="min-h-screen bg-[#050609] text-white">
      ...
      <section className="px-5 py-16 md:px-8 md:py-20">
        <div className="mx-auto max-w-7xl">
          ...
          {verifiedTorqueLinks.length > 0 ? (
            <div className="mt-10 border-t border-white/10 pt-10">
              <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.32em] text-emerald-300/80">
                    Verified Guides
                  </p>
                  <h2 className="mt-3 text-2xl font-black tracking-tight md:text-4xl">
                    Source-backed wheel torque pages.
                  </h2>
                </div>
                <p className="max-w-xl text-sm leading-6 text-white/45">
                  These pages are generated from verified Torque Hub rows with attached source details.
                </p>
              </div>

              <div className="mt-6 grid gap-4 md:grid-cols-3">
                {verifiedTorqueLinks.map((item) => (
                  <Link
                    key={item.href}
                    href={{ pathname: item.href }}
                    className="group rounded-[1.2rem] border border-white/10 bg-white/[0.025] p-5 transition hover:border-emerald-400/45 hover:bg-emerald-400/[0.06]"
                  >
                    ...
                    <span className="mt-4 inline-block text-xs font-bold text-red-400 transition group-hover:text-red-300">
